...
for i in range(n_elec): coh[0, i, i] = 0.
imcoh = ImCoh.fit_transform(eeg[np.newaxis, ...]).mean(axis=-1)
for i in range(n_elec): imcoh[0, i, i] = 0.

fig, axes = plt.subplots(1, 3)
axes[0].imshow(cov[0], cmap="Purples", )
...
axes[1].imshow(np.atleast_2d(coh[0][up_idx]), cmap="Oranges", vmin=0.)
axes[2].imshow(np.atleast_2d(imcoh[0][up_idx]), cmap="Greens", vmin=0.)
plt.show()
