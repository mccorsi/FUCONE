...
# datasets = [BNCI2015004()] # [BNCI2014001()] # BNCI2014002(), AlexMI()] #, Zhou2016()]  # rf PhysionetMI(), Schirrmeister2017()
# datasets = [BNCI2015001(), BNCI2015004(), BNCI2014001(), Zhou2016()]  # rh/f
datasets = [BNCI2014001(), Zhou2016(), BNCI2014004()] # Lee2019_MI, Lee2019_MI rh/lh
...
for d in datasets:
    ...
    for f in freqbands:
        ...
        for subject in tqdm(subjects, desc="subject"):
            ...
            for ppn, ppl in tqdm(all_ppl.items(), total=len(all_ppl), desc="pipelines"):
                cv = LeaveOneGroupOut()
                le = LabelEncoder()
                ...
                for train, test in cv.split(X, y, groups):
                    ...
                    acc = balanced_accuracy_score(y[test], yp)
                    ...
                    if isinstance(ppl, StackingClassifier):
                        for est_n, est_p in cvclf.named_estimators_.items():
                            ...
                            acc = balanced_accuracy_score(y[test], ype)
    ...
    dataset_res.to_csv(
        path_csv_root + "/OptEnsemble-coreFC-CrossSession-memory-2class-" + "-".join(events) + ".csv"
    )
    print("saving " + path_csv_root + "/OptEnsemble-coreFC-CrossSession-memory-2class-" + "-".join(events) + ".csv")
